...
x = torch.rand(2, 3, 768)  # Creates sample input with batch dimension 2
out = ffn(x)
# print(out.shape)
...
torch.manual_seed(123)  # Specifiesrandomseed for the initial weights for reproducibility
...
from exercise.chapter03 import MultiHeadAttention
...
def generate_text_simple(model, idx, max_new_tokens, context_size): # idx is a (batch, n_tokens) array of indices in the current context.
    for _ in range(max_new_tokens):
        ...
        logits = logits[:, -1, :]  # Focuses only on the last time step, sothat(batch,n_token,vocab_size) becomes (batch, vocab_size)
